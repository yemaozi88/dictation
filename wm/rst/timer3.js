...
function clickButton_show(qNum, qSet){
    tid = setInterval('countTime()', 10); 
    show_question_text(qNum, qSet);
}

function show_question_text(qNum, qSet){
    aTF_i = "aTF" + qNum;
	...
	= "<form id=\"formTF\">\n\
<input type='radio' id='" + aTF_i + "' name='" + aTF_i + "' value='1' onclick='clickButton_next(" + qNum + ',' + qSet + "); ' />正しい\n\
<input type='radio' id='" + aTF_i + "' name='" + aTF_i + "' value='0' onclick='clickButton_next(" + qNum + ',' + qSet + "); ' />誤り\n\
</form>";
    //document.getElementById("goNext").disabled = false;
    document.getElementById("TFall").innerHTML = "";
}

function clickButton_next(qNum, qSet){
    ...
    if(qNum <= qSet){
        document.getElementById("qTitle").innerHTML = qSet + " 文中 第　" + qNum + " 文目</center>";	        
        //newAttribute = "clickButton_next(" + qNum + ", " + qSet + ")";
        //document.getElementById("goNext").setAttribute("onclick", newAttribute);
    }else{
        document.getElementById('showbutton').disabled = true;
        show_question_last_word(qSet);
    }
 
    // update the button.
    //document.getElementById('showbutton').disabled = true;    
    var newAttribute = "clickButton_show(" + qNum + ", " + qSet + ")";
    document.getElementById("showbutton").setAttribute("onclick", newAttribute);  
}


function show_question_last_word(num){
    for (var i=1; i<=num; i++){
	wNum = "lastWord" + i;
